...
def step_when_i_send_a_request_to_the_endpoint(context, method):
    ...
    try:
        context.response_saver.save_response_json(context.response, filename)
...
# def step_given_i_have_the_payload(context):
#     context.payload = context.text
#
# @when('I send a "{method}" request to the endpoint')
# def step_when_i_send_a_request_to_the_endpoint(context, method):
#     context.response = context.api_factory.send_request(method, context.payload if hasattr(context, 'payload') else None)
#
# @then('the status code should be {expected_status_code:d}')
# def step_then_the_status_code_should_be(context, expected_status_code):
#     actual_status_code = context.api_factory.get_status_code()
#     assert actual_status_code == expected_status_code, (
#         f"Expected status code {expected_status_code}, but got {actual_status_code}"
#     )
#
